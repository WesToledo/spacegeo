...
class XRGestures extends THREE.EventDispatcher {
  constructor(renderer) {
    ...
    function onSelectEnd() {
      ...
      if (self.type === "swipe") {
        const direction =
          self.controller1.position.y < data.startPosition.y ? "DOWN" : "UP";
        ...
        if (startToEnd < self.doubleClickLimit) {
          ...
          self.dispatchEvent({
            type: "press",
            position: self.controller1.position,
            matrixWorld: self.controller1.matrixWorld,
          });
          self.type = "unknown";
        }
      } else {
  ...
  get multiTouch() {
    ...
    if (this.controller1 === undefined || this.controller2 === undefined) {
      ...
      result =
        this.controller1.userData.selectPressed &&
  ...
  update() {
        ...
        switch (data1.taps) {
          case 1:
            ...
            this.dispatchEvent({
              ...
              matrixWorld: this.controller1.matrixWorld,
            });
            break;
          case 2:
            this.dispatchEvent({
    ...
    if (this.type === "unknown" && this.touch) {
      if (data1.startPosition !== undefined) {
        if (this.multiTouch) {
          if (data2.startPosition !== undefined) {
            ...
            const startDistance = data1.startPosition.distanceTo(
              data2.startPosition
            );
      ...
      const v = this.controller2.position
        .clone()
        .sub(this.controller1.position)
        .normalize();
